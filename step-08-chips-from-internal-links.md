# Step 08 — Chips from Internal Doc Links
...
```
TASK
Convert internal doc URLs to chips:
- On paste or `Cmd/Ctrl+K` insertion, detect internal URLs, fetch preview (title, last editor, updatedAt).
- Render chip with hover card; action to revert to plain link.

CONSTRAINTS
- RBAC-aware preview: if caller lacks access, render anonymized token and no metadata.

DELIVERABLES
- Chip node/mark + preview provider (mocked in tests).
- Tests:
  - C.1 `chip-autoconvert`, `chip-hover`, `chip-revert`

ACCEPTANCE
- All C.1 tests pass; anonymized behavior respected when access is missing.
```
