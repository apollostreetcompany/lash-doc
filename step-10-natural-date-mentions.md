...
```
TASK
Add natural language date mentions:
- `@next Friday 3pm` → date chip with ISO timestamp (user timezone), original display retained.
- Hover shows absolute date/time; respect locale.

CONSTRAINTS
- Deterministic timezone in tests (mock TZ).
- Parsing library must be locale-aware and testable.

DELIVERABLES
- Date chip parser/renderer; locale utilities.
- Tests:
  - C.3 `mention-date-parse`, `mention-date-locale`

ACCEPTANCE
- Both tests green with fixed TZ/locale fixtures.
```
