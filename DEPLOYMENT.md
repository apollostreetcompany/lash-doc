...
- Room socket protocol: `yjs-update` messages can include `updateId` and receive `sync-ack`; document content, comment thread metadata, and suggestion resolution records all travel as Yjs updates. `awareness-update` messages carry label/color/selection and receive room-scoped `awareness-state` peer lists.
