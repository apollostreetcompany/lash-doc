# Step 04 — Markdown Import/Export
...
- Import: headings, lists, checklists (`- [ ]` / `- [x]`), images (`![alt](src)`), and GitHub-style pipe tables → table node with text cells.
- Export: produce Markdown that round-trips structure (images as references; tables as pipes).
- Wire UI actions for Import (.md file) and Export.

CONSTRAINTS
- Do not lose inline marks (bold/italic/code) during round-trip.
- Gracefully degrade unsupported constructs with warnings.

DELIVERABLES
- Parser/serializer in packages/editor-core/markdown.
- E2E tests:
  - A.3 `md-roundtrip-basic`, `md-table-import`

ACCEPTANCE
- Round-trip preserves structure for headings/lists/checklists/images/tables.
- Both tests pass.
```
