...
test('chat-orphan', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const editor = (window as unknown as { __lashEditor: import('@tiptap/core').Editor })
      .__lashEditor;
    editor.chain().focus().setTextSelection({ from: 8, to: 14 }).deleteSelection().run();
  });

  await expect(page.getByTestId('chat-orphaned')).toContainText('Orphaned');
  await expect(page.getByTestId('chat-current-context')).toContainText('Context lost');
});
