...
test('chat-history-context', async ({ page }) => {
  await page.goto('/');
  await ready(page);
  ...
  await expect(page.getByTestId('chat-history-context')).toContainText('target');
  await expect(page.getByTestId('chat-anchor-text')).toContainText('target');
});
