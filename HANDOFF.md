...
- Active follow-up branch: `codex/ux/delightful-writing-followup` in worktree `/Users/borker/dev/lash-doc-delight-followup`.
- OUT-01 follow-up is implemented on the follow-up branch: the desktop primary outline now sits beside the document canvas, the global sidebar defaults to an icon rail, duplicate outline landmarks are avoided with sidebar-specific IDs/fallback rendering, and mobile drawer outline access is preserved.
