...
- This applies to fail-first capture too: before running a new hook-dependent Playwright regression, rebuild with `NEXT_PUBLIC_LASH_TEST_HOOKS=true pnpm --filter @lash/web build` even if a previous bead already built with hooks.
- Do not run multiple `pnpm run test:e2e -- ...` commands in parallel in the same worktree. Each command builds and starts a Playwright `webServer` on port 3000, so parallel runs can collide on the port and `.next` cache even when the product behavior is fine.
- Do not pass nullable unquoted globs such as `.env*` or `next.config.*` to zsh search commands. zsh rejects a command before `rg` runs when no path matches; enumerate known directories, derive candidates with `rg --files`, or quote the pattern.
