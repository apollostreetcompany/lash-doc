# Step 13 — History Log + Snapshots + Restore
...
TASK
Persist history:
- Append-only Yjs update log with metadata (actorId, ts, intent).
- Periodic snapshots (size- or time-based).
- History UI: sessions grouped by time; Restore creates a new head.

CONSTRAINTS
- Non-destructive restore (no rewrite of history).
- Migration SQL for Postgres tables: doc_updates, doc_snapshots.

DELIVERABLES
- Persistence layer + UI wiring + SQL migrations.
- Tests:
  - D.2 `history-open`, `history-diff` (smoke render), `history-restore`

ACCEPTANCE
- Timeline shows sessions; restore results in a new head version.
```
